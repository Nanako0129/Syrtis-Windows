...
| Area | Convention |
|---|---|
| Branch name | `<type>/<kebab-summary>`, where `type` is the commit's Conventional Commit prefix (`feat`, `fix`, `docs`, `chore`, `refactor`, `test`, `ci`, `build`, `perf`, or any other prefix this repository's commits already use — the list is not closed), such as `fix/grok-weekly-credit-pool`. One reviewable concern per branch. |
